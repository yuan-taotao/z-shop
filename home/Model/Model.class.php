<?php
	class Model{
		...
		function __construct($tabName = '',$cache = './cache/'){
			...
			//判断是否传入表名，如果传入表名则用传入的，如果没有传入表名呢 需要 自己去获取表名
			if($tabName == ''){
				//没有传入表名 需要从子承的类中获取表名
				//$a = get_class($this);
		...
		//4.实现发送添加、删除、以及修改的SQL语句(增、删、改返回受影响)
		private function exec($sql){
			...
			if($result && mysqli_affected_rows($this->link)>0){
				//成功 如果有上一步操作的iｄ 则返回上一步操作的id 如果没有上一步操作的id 则返回受影响行
		...
		public function delete(){
			...
			if(!empty($this->where)){
				...
				$where = '';
				//如果用户没有传入删除条件，自动判断get数组中是否有主键，如果有则按该键为删除条件
				if(!empty($_GET)){
					//先获取缓存字段
					$field = $this->getField();
					//在缓存字段中获取主键
